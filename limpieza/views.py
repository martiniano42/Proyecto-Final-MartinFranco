...
def limpiezas(request):
    ...
    return render(request, 'limpieza/limpiezas.html', {'formulario': formulario, 'limpiezas': listado_de_limpieza})
...
@login_required
def eliminar_limpieza(limpieza_id):
...
def actualizar_limpieza(request, limpieza_id):
    ...
    if request.method == "POST":
        formulario = ActualizarLimpiezaFormulario(request.POST, request.FILES)
        ...
        if formulario.is_valid():
            ...
            return render(request, 'limpieza/actualizar_limpieza.html', {'formaulario': formulario})
  
    
    
    formulario = ActualizarLimpiezaFormulario(initial={'producto': limpieza_a_actualizar.producto,'marca': limpieza_a_actualizar.marca, 'descripcion': limpieza_a_actualizar.descripcion,'vencimiento': limpieza_a_actualizar.vencimiento})
    return render(request, 'limpieza/detalle_limpieza.html', {'formulario': formulario})

def detalle_limpieza(request, limpieza):
    limpieza = Limpieza.objects.get(id=limpieza.id)
